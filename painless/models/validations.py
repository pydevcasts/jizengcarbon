...
def validate_national_code(value):
    if not len(value) == 10:
        raise forms.ValidationError('The national code is invalid')
...
def validate_postal_code(value):
    if not len(value) == 10:
        raise forms.ValidationError('Zip code is invalid')
...
def validate_file_extension(value):
    ...
    if not ext.lower() in valid_extensions:
        raise forms.ValidationError('formate entered is invalid')
...
def validate_file_size(value):
    ...
    if filesize > 10485760:
        raise forms.ValidationError("You can not upload a file greate than 100 mb")
    else:
        return value